...
plot(NA, NA, xlim = c(-2,2), ylim = AlphaRange, main = "", xlab = "Environment", 
...
xCoords <- c(env.seq, env.seq[n:1])
yCoords <- c(GenericPlot[2,], GenericPlot[3,n:1])
polygon(x = xCoords, y = yCoords, border = NA, col = "grey")
lines(x = env.seq, y = GenericPlot[1,], lty = 1, lwd = 1.5)
# Add the lines for the intraspecific term
lines(x = env.seq, y = IntraPlot[1,], lwd = 1.5, col = IntraCol)
lines(x = env.seq, y = IntraPlot[2,], lty = 2, col = IntraCol)
lines(x = env.seq, y = IntraPlot[3,], lty = 2, col = IntraCol)
# Now the deviating alphas
for(i in 1:2){
        lines(x = env.seq, y = SpecificPlot[i,1,], lwd = 1.5, col = AlphaCols[i])
        lines(x = env.seq, y = SpecificPlot[i,2,], lty = 2, col = AlphaCols[i])
        lines(x = env.seq, y = SpecificPlot[i,3,], lty = 2, col = AlphaCols[i])
}
...
patchwork <- p.mono + p.opt

patchwork + 
        plot_annotation(tag_levels = 'a')  & 
        theme(plot.tag = element_text(face = "bold"))

# ggsave(filename = here('BH_simulations/Box/box_figure_lambda.pdf'), width = 10, height = 4, units = 'in')


### Unused: previos Box figure part 2: different options for alpha------------
...
a.terms <- ggplot(alpha.fit.long, 
        ...
        scale_color_manual(values = c(InterceptCol, SlopeCol, SlopeCol), name = "",
                           labels = c(expression(Simple~model:~alpha[ij]~pairs), 
                                      expression(Complex~model:~alpha[ij]~pairs),
                                      expression(Complex~model:~all~terms)),
