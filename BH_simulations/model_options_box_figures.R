...
num.datasets <- 2
ppcPreds <- vector(mode = "list", length = num.datasets)
Alphas <- vector(mode = "list", length = num.datasets)
Lambdas <- vector(mode = "list", length = num.datasets)
InterceptInclusion <- vector(mode = "list", length = num.datasets)
GenericDeviations <- vector(mode = "list", length = num.datasets)

# Now population the objects
FilePrefixes <- c("N10_", "N50_")
for(i in 1:num.datasets){
     FileName <- paste("StanFits/monoLambda_constAlpha/",
     ...
     #      generic alpha terms
     InterceptDev <- Posteriors$alpha_generic - TrueGenericIntercept
     GenericDeviations[[i]] <- matrix(data = NA, nrow = 1, ncol = 3)
     GenericDeviations[[i]][1] <- mean(InterceptDev)
     GenericDeviations[[i]][2:3] <- HDInterval::hdi(InterceptDev)
}
...
AllNonGeneric <- which(InterceptInclusion[[num.datasets]] == 1)
...
FigName <- "Results/monoLambda_constAlpha/SimResults.pdf"
...
for(i in 1:num.datasets){
     ...
     points(x = GenericDeviations[[i]][1], Generic_y[1], pch = InterceptPoints, col = InterceptCol)
    # points(x = GenericDeviations[[i]][2,1], Generic_y[2], pch = SlopePoints, col = SlopeCol)
     segments(x0 = GenericDeviations[[i]][2], y0 = Generic_y[1], x1 = GenericDeviations[[i]][3],
   ...
   #           y1 = Generic_y[2], col = SlopeCol)
     ...
     for(j in 1:NumNonGeneric){
          if(InterceptInclusion[[i]][AllNonGeneric[j]] == 1){
               points(x = Alphas[[i]][1,AllNonGeneric[j]], y = NonGeneric_y[j], pch = InterceptPoints,
                      col = InterceptCol)
               segments(x0 = Alphas[[i]][2,AllNonGeneric[j]], y0 = NonGeneric_y[j],
                        x1 = Alphas[[i]][3,AllNonGeneric[j]], y1 = NonGeneric_y[j], col = InterceptCol)
          }
     ...
     points(x = Alphas[[i]][1,Focal], Intra_y[1], pch = InterceptPoints, col = InterceptCol)
   #  points(x = Alphas[[i]][1,2,Focal], Intra_y[2], pch = SlopePoints, col = SlopeCol)
     segments(x0 = Alphas[[i]][2,Focal], x1 = Alphas[[i]][3,Focal], y0 = Intra_y[1], y1 = Intra_y[1], col = InterceptCol)
   #  segments(x0 = Alphas[[i]][2,2,Focal], x1 = Alphas[[i]][3,2,Focal], y0 = Intra_y[2], y1 = Intra_y[2], col = SlopeCol)
     ...
     plot(x = NA, y = NA, 
          xlim = ppc_xRange, ylim = ppc_yRange, 
          xlab = "", ylab = "", las = 1)
     mtext("Predicted growth", side = 2, line = 2.5)
     #axis(1, at = seq(-2, 2, by = 0.25), tcl = -0.25, labels = FALSE)
     #axis(2, at = seq(-2, 2, by = 0.25), tcl = -0.25, labels = FALSE)
