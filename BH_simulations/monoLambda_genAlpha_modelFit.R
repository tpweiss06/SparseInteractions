...
Focal <- 1
PrelimStanPath <- "StanCode/Prelim_monoLambda_genAlpha.stan"
FinalStanPath <- "StanCode/Final_monoLambda_genAlpha.stan"

# Load in the appropriate data
FullSim <- read.csv("Simulations/simulation_perturb2_genAlpha.csv")
TrueVals <- read.csv("Simulations/parameters_perturb2_genAlpha.csv")
TrueAlphaMeans <- TrueVals$alpha.1
TrueAlphaSlopes <- TrueVals$alpha.env.gen
...
ChainInitials <- list(lambdas = c(TrueVals$lambda.mean[Focal], TrueVals$lambda.env[Focal]), 
                      ...
                      alpha_intra_tilde = TrueAlphaMeans[Focal])
...
FitFileName <- paste("StanFits/monoLambda_genAlpha/", FilePrefix, "PrelimFit.rdata", sep = "")
...
FitFileName <- paste("StanFits/monoLambda_genAlpha/", FilePrefix, "FinalFit.rdata", sep = "")
