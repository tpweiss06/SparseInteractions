# This script will create the figure showing model performance across 1,000 independent
#    simulations and 3 different input data amounts.

rm(list = ls())
library(RColorBrewer)
library(here)

results <- read.csv(here("BH_simulations/Main/StanFits/MainSimFits.csv"))
...
FigName <- here("BH_simulations/Main/AllSimResults.pdf")
pdf(file = FigName, width = 14, height = 4, onefile = FALSE, paper = "special")
     ...
     plot(x = NA, y = NA, xlim = xRange, ylim = yRanges[[1]], main = "", xlab = "Size",
          ylab = "Parameter deviation", las = 1, axes = FALSE, cex.lab = 1.5)
     box()
     axis(side = 1, at = 1:3, tcl = -0.5, labels = sizes, cex.axis = 1.5)
     ...
     # Add the boxes
     for(p in 1:8){
          boxplot(results[,2+p] ~ results$Size, add = TRUE, at = xSeqs[[1]][,p], boxwex = 0.075,
    ...
    # Now add the plot for the number of non-generic species
     plot(x = NA, y = NA, xlim = xRange, ylim = yRanges[[2]], main = "", xlab = "Size",
          ylab = "Number of species", las = 1, axes = FALSE, cex.lab = 1.5)
     box()
     axis(side = 1, at = 1:3, tcl = -0.5, labels = sizes, cex.axis = 1.5)
     axis(side = 2, at = 0:9, tcl = -0.5, las = 1, cex.axis = 1.5)
     boxplot(results$NumNonGenericInt ~ results$Size, add = TRUE, at = xSeqs[[2]][,1], boxwex = 0.125,
             axes = FALSE, col = InterceptCol)
     boxplot(results$NumNonGenericSlope ~ results$Size, add = TRUE, at = xSeqs[[2]][,2], boxwex = 0.125,
             axes = FALSE, col = SlopeCol)
     boxplot(results$NumNonGenericSpecies ~ results$Size, add = TRUE, at = xSeqs[[2]][,3], boxwex = 0.125,
             axes = FALSE)
     ...
     boxplot(results$RMSE ~ results$Size, col = RMSECol, xlab = "Size", ylab = "RMSE", main = "",
