...
transformed parameters{
  ...
  for(i in 1:2){
    ...
    lambdas[i,1] = 10 * lambdas_tilde[i,1];
    lambdas[i,2] = 10 * lambdas_tilde[i,2];
    for(s in 1:S){
      alpha_hat_ij[i,s] = 10 * alpha_hat_ij_tilde[i,s];
      alpha_hat_eij[i,s] = 10 * alpha_hat_eij_tilde[i,s];
    }
  }
}
...
model{
  ...
  for(i in 1:N){
    ...
    for(s in 1:S){
      if(Inclusion_ij[reserve[i],s] == 1){
        if(Inclusion_eij[reserve[i],s] == 1){
          alpha_eij[i,s] = exp(alphas[1] + alpha_hat_ij[reserve[i],s] + (alphas[2] + alpha_hat_eij[reserve[i],s]) * env[i]);
        }else{
          alpha_eij[i,s] = exp(alphas[1] + alpha_hat_ij[reserve[i],s] + alphas[2] * env[i]);
        }
      }else{
        if(Inclusion_eij[reserve[i],s] == 1){
          alpha_eij[i,s] = exp(alphas[1] + (alphas[2] + alpha_hat_eij[reserve[i],s]) * env[i]);
        }else{
